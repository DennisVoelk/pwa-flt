...
export class TorchCe extends BaseCe {
    ...
    if (noActiveTrack) {
      if (renewIfNecessary) {
        this.trackInfoPromise = WIN.navigator.mediaDevices.
          ...
          then(async track => ({
            hasTorch: !!track && await this.utils.waitAndCheck(100, 25, () => !!track.getCapabilities().torch), //Prüfen ob Flash vorhanden/ob Permissions gegeben wurden
            track,
          }));
  ...
  protected async initialize(): Promise<IInitializedCe<this>> {
    ...
    const updateState = this.updateState = async (newState: State, extraMsg?: string) => {
      ...
      if (track) track.applyConstraints({advanced: [{torch: on}]}); //Flash an machen
    };
